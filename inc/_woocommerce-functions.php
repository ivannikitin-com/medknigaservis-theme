...
add_action( 'woocommerce_single_product_summary', 'add_buy_grouped_button', 6 );
add_action( 'woocommerce_after_add_to_cart_form', 'add_buy_grouped_button', 1 );
function add_buy_grouped_button(){
	global $product;
...
function med_add_custom_price_for_grouped_product() {
	...
	if ($product->is_type('grouped')) {	
		...
		woocommerce_wp_text_input( array(
		   'id'                => '_external_grouped_price',
		   'label'             => __( 'Цена комплекта', 'woocommerce' ),
		   'placeholder'       => '',
		   'description'       => __( 'Цена комплекта, который можно купить на стороннем сайте', 'woocommerce' ),
		) );
	}
}

add_filter( 'woocommerce_grouped_price_html', 'med_add_grouped_price', 10, 3 );
function med_add_grouped_price( $price, $product, $child_prices ) {
/*$external_grouped_price = get_post_meta($product->get_id(),'_external_grouped_price',true);
if ($external_grouped_price) { $price = wc_price($external_grouped_price); }
return $price;*/
return '';
}
...
/*****************************************************************/
/* Добавление фильтра по payment_method к таблице заказов в админке */
/*****************************************************************/
function med_filter_orders_by_payment_method() {
		global $typenow;
		...
		if ( 'shop_order' === $typenow ) {
			...
			if ( count( $methods ) ) : ?>
				<select name="_payment_methods" id="dropdown_payment_methods">
					<option value="">
						<?php esc_html_e( 'Фильтрация по методам оплаты', 'wc-filter-orders' ); ?>
					</option>
					<?php foreach ( $methods as $method ) : ?>
						<option value="<?php echo esc_attr( $method->id ); ?>" <?php echo esc_attr( isset( $_GET['_payment_methods'] ) ? selected( $method->method_title, $_GET['_payment_methods'], false ) : '' ); ?>>
...
function med_filter_request( $vars ) {
    global $pagenow, $typenow;

    // Only on WooCommerce admin orders list
    if ( $pagenow == 'edit.php' && 'shop_order' === $typenow && isset( $_GET['_payment_methods'] ) && ! empty( $_GET['_payment_methods'] ) ) {
        $vars['meta_key']   = '_payment_method';
        $vars['meta_value'] = $_GET['_payment_methods'];
...
if ( is_admin() && ! defined( 'DOING_AJAX' ) ) {
	add_action( 'restrict_manage_posts', 'med_filter_orders_by_payment_method' );
	add_filter( 'request', 'med_filter_request', 10, 1 );
}
...
//add_filter( 'woocommerce_product_get_image', 'med_placeholder_image', 10);
/*function med_placeholder_image($image){
...
function woocommerce_get_product_thumbnail( $size = 'woocommerce_thumbnail', $deprecated1 = 0, $deprecated2 = 0 ) {
	...
	return $product ? $product->get_image( $image_size, array(), true ) : '';
}